...
export default async function TrendingHeadlines({}) {
  const trendingPostsInfo = await getTrendingPostsInfo();

  async function getFinalTrendingPosts(array) {
    const trendingPosts = [];
    for (let i = 0; i < array.length; i++) {
      const post = await sanityFetch({query: `*[_type == "article" && slug.current == "${array[i]._id}"]{_id, name, slug, tags, publishedAt}|order(date desc)[0...4]`});
      ...
      <div className="w-full h-full grid grid-cols-1 justify-start gap-5">
        {trendingPosts.map((post, index) => (
          <div
            key={index}
            className={clsx(
              "flex items-start justify-start gap-4 pb-4 h-[6.5rem]",
              {
          ...
          >
            <div className="w-1/6 h-full flex items-center justify-start">
              <span className="trending-number text-4xl sm:text-5xl font-bold drop-shadow-[0_1px_0px_rgba(0,0,0,0.5)] ">
                #{index + 1}
              </span>
            </div>
            <div className="w-5/6 h-full flex flex-col items-start justify-between gap-3 ">
              ...
              >
                <span className="text-lg sm:text-xl text-green-700 hover:text-opacity-80 font-bold drop-shadow-[1px_1px_0px_rgba(0,0,0,0.25)]">
                  {post.name}
                </span>
              </Link>
